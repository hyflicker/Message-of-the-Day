package com.hyflicker.MOTD.Commands.Config.WelcomeBanner;
...
public class WelcomeBanner extends CommandBase {

    public WelcomeBanner() {
        super("welcomeBanner", "Gets and Sets the welcomeBanner in the config file.");
    ...
    protected void executeSync(@Nonnull CommandContext ctx) {
        ...
        if (rawCommandMessage.length < 4) {
            ctx.sendMessage(Message.raw("Usage: /motd config welcomeBanner [primaryTitle, secondaryTitle, isMajor, duration, fadeInDuration, fadeOutDuration, randomizeTitleFile, enabled]").color(Color.red));
            return;
        }
        ...
        switch (command) {
            ...
            case "secondaryTitle":
                if (!value.isBlank()) {
                    config.setSecondaryTitle(value);
                    sendCommandSetMessage(command, value, ctx);
                } else {
                    ctx.sendMessage(
                            Message.join(
                                    Message.raw(command + ": ").color(Color.green),
                                    Message.raw(config.getSecondaryTitle())
                            )
                    );
                }
                break;
            ...
            case "randomizeTitleFile":{
                if (!value.isBlank()) {
                    ...
                    config.setRandomizeTitle(Boolean.parseBoolean(value));
                    sendCommandSetMessage(command, value, ctx);
                } else {
                    ctx.sendMessage(
                            Message.join(
                                    Message.raw(command + ": ").color(Color.green),
                                    Message.raw(Boolean.toString(config.getRandomizeTitle()))
                            )
                    );
                }
            }
